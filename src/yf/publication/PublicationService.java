...
public class PublicationService {
    ...
    public PublicationElasticDTO getPublicationById(final String publicationId) {
        ...
        GetResponse res = nativeElastiClient.getClient()
                .prepareGet()
    ...
    public PublicationElasticDTO getPublicationByVkPostId(final String postId) {
        ...
        boolQuery.must(QueryBuilders.termQuery("vkPost.id",
                postId));
        ...
        return ElasticToObjectConvertor.convertSingleResultToObjectFromResonse(res,
                PublicationElasticDTO.class);
    ...
    public PublicationElasticDTO getPublicationByLink(final String link) {
        ...
        BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
        boolQuery.must(QueryBuilders.termQuery("link",
                link));
    ...
    public List<SharedBasicPostDTO> getPublicationsByTypeFromTo(final PublicationTypeEnum typeEnum,
        ...
        SearchRequestBuilder request = nativeElastiClient.getClient()
                .prepareSearch(properties.get("elastic.index.publication"))
                .setTypes(properties.get("elastic.type.photo"))
                .addSort("date",
                        SortOrder.DESC)
        ...
        if (typeEnum != null) {
            BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
            boolQuery.must(QueryBuilders.matchQuery("type",
                    typeEnum.toString()));
    ...
    public List<SharedBasicPostDTO> searchRelated(final PublicationElasticDTO publicationElasticDTO) {
        ...
        BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
        boolQuery.mustNot(QueryBuilders.termQuery("id",
                publicationElasticDTO.getId()));
        ...
        if (relatedPubs.size() <= 3) {
            relatedPubs.addAll(getPublicationsByTypeFromTo(null,
                    0,
                    5));
        }
        return relatedPubs;
    ...
    public List<SharedBasicPostDTO> searchPublications(final String queries) {
        ...
        for (String query : splitedQuery) {
            boolQuery.should(QueryBuilders.matchPhraseQuery("mdSimple",
                    query));
            boolQuery.should(QueryBuilders.matchPhraseQuery("phSimple",
                    query));
        }
    ...
    public List<SharedBasicPostDTO> getUserPublications(final String user_id,
                                                        final int from,
                                                        final int size) {
    ...
    private BoolQueryBuilder buildUserMatchPhMd(final String userId) {
        ...
        boolQuery.should(QueryBuilders.termQuery("phUsers.userId",
                userId));
        boolQuery.should(QueryBuilders.termQuery("mdUsers.userId",
                userId));
    ...
    private BoolQueryBuilder buildRelatedQueryByMdPh(final PublicationElasticDTO publicationElasticDTO) {
        ...
        if (publicationElasticDTO.getPhUsers() != null) {
            publicationElasticDTO.getPhUsers()
                    .forEach(phUser -> {
                        boolQuery.should(QueryBuilders.termQuery("phUsers.profileId",
                                phUser.getProfileId()));
                    });
        }
        if (publicationElasticDTO.getMdUsers() != null) {
            publicationElasticDTO.getMdUsers()
                    .forEach(mdUser -> {
                        boolQuery.should(QueryBuilders.termQuery("mdUsers.profileId",
                                mdUser.getProfileId()));
                    });
        }
        if (publicationElasticDTO.getMdSimple() != null) {
            boolQuery.should(QueryBuilders.matchPhraseQuery("mdSimple",
                    publicationElasticDTO.getMdSimple()));
        }

        if (publicationElasticDTO.getPhSimple() != null) {
            boolQuery.should(QueryBuilders.matchPhraseQuery("phSimple",
                    publicationElasticDTO.getPhSimple()));
        }
