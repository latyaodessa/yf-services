...
public class ProfilesDao {
    ...
    public PhProfile getPhProfileById(final Long id) {
        return em.find(PhProfile.class, id);
    }

    public MdProfile getMdProfileById(final Long id) {
        return em.find(MdProfile.class, id);
    }

    public PhProfile getPhProfileByUserId(final Long userId) {

        TypedQuery<PhProfile> query = em.createNamedQuery(PhProfile.QUERY_GET_PG_PROFILE_BY_USER_ID, PhProfile.class)
    ...
    public MdProfile getMdProfileByUserId(final Long userId) {
        TypedQuery<MdProfile> query = em.createNamedQuery(MdProfile.QUERY_GET_MD_PROFILE_BY_USER_ID, MdProfile.class)
                .setParameter("user_id", userId);
        try {
            return query.getSingleResult();
        } catch (NoResultException e) {
