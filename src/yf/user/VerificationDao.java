...
public class VerificationDao {
    ...
    public Verifications getVerificationByVerification(final String verification) {
        ...
        final Date yersterday = new DateTime(new Date()).minusDays(1)
                .toDate();

        TypedQuery<Verifications> query = em.createNamedQuery(Verifications.QUERY_GET_VER_BY_VERFICATION,
                Verifications.class)
                .setParameter("verification",
                        verification)
                .setParameter("yersterday",
                        yersterday);
        try {
