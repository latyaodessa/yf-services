...
public class VkRestClient {
    ...
    private static final String VK_REQUIRED_FIELDS = "&fields=" + String.join(",",
            new String[] {SEX_FIELD,
                          MIDDLENAME_FIELD,
                          BDAY_FIELD,
                          CITY_FIELD,
                          COUNTRY_FIELD,
    ...
    public VKUserDTO getVKUserDetails(final long userId) {

        String JSONdto = Client.create()
                .resource(userDetailsURI(userId))
        ...
        try {
            ...
            if (response.has("response")) {
                JSONObject jsonObject = new JSONObject(JSONdto).getJSONArray("response")
                        .getJSONObject(0);
    ...
    private VKUserDTO generateVkUser(final JSONObject jsonObject) {
        ...
        dto.setId(jsonService.getLongFromJsonObject(jsonObject,
                ID_FIELD));
        dto.setFirstName(jsonService.getStringFromJsonObject(jsonObject,
                FIRSTNAME_FIELD));
        dto.setLastName(jsonService.getStringFromJsonObject(jsonObject,
                LASTNAME_FIELD));
        dto.setSex(jsonService.getStringFromJsonObject(jsonObject,
                SEX_FIELD));
        dto.setBdate(jsonService.getStringFromJsonObject(jsonObject,
                BDAY_FIELD));
        dto.setVerified(jsonService.getIntegerFromJsonObject(jsonObject,
                VERIIFIED_FIELD) != null
                && jsonService.getIntegerFromJsonObject(jsonObject,
                        VERIIFIED_FIELD) != 0);

        JSONObject counryData = jsonService.getJsonObjectFromJsonObject(jsonObject,
                COUNTRY_FIELD);
        if (counryData != null) {
            dto.setCountry(jsonService.getStringFromJsonObject(counryData,
                    TITLE_FIELD));
        }

        JSONObject cityData = jsonService.getJsonObjectFromJsonObject(jsonObject,
                CITY_FIELD);
        if (cityData != null) {
            dto.setCity(jsonService.getStringFromJsonObject(cityData,
                    TITLE_FIELD));
        }
        return dto;
    }

    private String userDetailsURI(final long userId) {
