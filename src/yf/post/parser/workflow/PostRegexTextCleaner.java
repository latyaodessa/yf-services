...
public class PostRegexTextCleaner {
    ...
    public PostElasticDTO getCleanedText(Post entity,
                                         PostElasticDTO dto) {

        String ph_cleaned = executeRegex(entity.getText(),
                ph);
        String md_cleaned = executeRegex(entity.getText(),
                md);
    ...
    public String getCleanedPh(final String text) {
        String ph_cleaned = executeRegex(text,
                ph);
        if (!StringUtils.isBlank(ph_cleaned)) {
            ph_cleaned = removePhTag(ph_cleaned);
            return ph_cleaned;
    ...
    public String getCleanedMd(final String text) {
        String md_cleaned = executeRegex(text,
                md);
        if (!StringUtils.isBlank(md_cleaned)) {
            md_cleaned = removeMdTag(md_cleaned);
            return md_cleaned;
    ...
    private String removePhTag(String text) {
        return text.replaceFirst(ph_tag,
                "")
                .replaceAll(REGEX_CLEANER,
                        StringUtils.EMPTY)
                .trim();
    }

    private String removeMdTag(String text) {
        return text.replaceFirst(md_tag,
                "")
                .replaceAll(REGEX_CLEANER,
                        StringUtils.EMPTY)
                .trim();
    ...
    private String executeRegex(String text,
                                String regex) {
    ...
    public Set<Long> getPhIds(final String text) {
        final String ph_cleaned = executeRegex(text,
                ph);
        return getIds(ph_cleaned);
    }

    public Set<Long> getMdIds(final String text) {
        final String md_cleaned = executeRegex(text,
                md);
