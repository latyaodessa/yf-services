...
public class PostService {
    ...
    public PostDetailsDTO getPostDetailsDTO(final String postId) {
        ...
        MultiGetResponse res = nativeElastiClient.getClient()
                .prepareMultiGet()
                .add(properties.get("elastic.index.native"),
                        properties.get("elastic.type.photo"),
                        postId)
                .add(properties.get("elastic.index.sets"),
                        properties.get("elastic.type.photo"),
                        postId)
                .add(properties.get("elastic.index.art"),
                        properties.get("elastic.type.photo"),
                        postId)
                .get();
        ...
        for (MultiGetItemResponse itemResponse : res) {
            ...
            if (response.isExists()) {
                PostElasticDTO searchResult = ElasticToObjectConvertor.convertSingleResultToObject(response.getSourceAsString(),
    ...
    @Deprecated
    public List<SharedBasicPostDTO> getNewPostsFromTo(final String index,
                                                      final String type,
                                                      final int from,
                                                      final int size) {
    ...
    public List<SharedBasicPostDTO> getTopPostsFromTo(final String index,
                                                      final String type,
                                                      final int from,
                                                      final int size) {
    ...
    public List<PostElasticDTO> getElasticTopPostsFromTo(final String index,
                                                         final String type,
                                                         final int from,
                                                         final int size) {
    ...
    public List<SharedBasicPostDTO> searchPosts(final String queries) {
        ...
        SearchResponse res = nativeElastiClient.getClient()
                .prepareSearch(properties.get("elastic.index.native"),
                        properties.get("elastic.index.sets"),
                        properties.get("elastic.index.art"))
                .setQuery(getBoolQueryForMultipleSearch("text",
                        splitedQuery))
                .addSort("id",
                        SortOrder.DESC)
                .setFrom(0)
    ...
    public List<SharedBasicPostDTO> searchRelated(final String queries,
        ...
        SearchResponse res = nativeElastiClient.getClient()
                .prepareSearch(properties.get("elastic.index.native"),
                        properties.get("elastic.index.sets"))
                .setQuery(getBoolQueryForMultipleSearch("text",
                        splitedQuery).mustNot(
                                QueryBuilders.termQuery("id",
                                        excludeId)))
                .addSort("id",
                        SortOrder.DESC)
                .setExplain(true)
    ...
    private BoolQueryBuilder getBoolQueryForMultipleSearch(final String field,
                                                           final String[] splitedQuery) {
        BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
        for (String query : splitedQuery) {
            boolQuery.should(QueryBuilders.matchPhraseQuery(field,
                    query));
        }
