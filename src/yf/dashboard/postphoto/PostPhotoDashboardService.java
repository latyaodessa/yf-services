...
public class PostPhotoDashboardService {
    ...
    public PostDashboardElasticDTO saveNewPublicationForUser(final Long publicationId, final Long userId) {
        final PostDashboardElasticDTO savedPublication = getSavedPublication(userId, publicationId);
        if (savedPublication == null) {
            return postPhotoDashboardWorkflow.saveNewPublicationForUser(publicationId, userId);
        }
        return null;
    }

    public PostDashboardElasticDTO getSavedPublication(final Long user_id, final Long pubId) {
        return postPhotoDashboardWorkflow.findUserSavedPostByUserIdAndPublicationId(user_id, pubId);
    ...
    public List<PostDashboardElasticDTO> getSavedDashboardPosts(final String user_id, final int from, final int size) {
        ...
        final List<String> collect = savedDashboardPosts.stream().map(p -> Long.toString(p.getDto().getId())).collect(Collectors.toList());
        ...
        savedDashboardPosts.forEach(saved -> {
            SharedBasicPostDTO dto = publicationsByIds.stream().filter(pub -> pub.getId().equals(saved.getDto().getId())).findFirst().orElse(null);
            if (dto != null) {
                saved.setDto(dto);
            }
        });
    ...
    public void deletePostFromUser(final Long savedPubId) {
        ...
        if (isRemoved) {
            nativeElastic.getClient().prepareDelete(properties.get("elastic.index.dashboard.saved.post"),
