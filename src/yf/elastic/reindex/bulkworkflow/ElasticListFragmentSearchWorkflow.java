...
public abstract class ElasticListFragmentSearchWorkflow<T> {
    ...
    public static void getStringFieldsRecursively(final Class<?> clazz,
        ...
        getStringFieldsRecursively(clazz,
                stringFields,
                "");
    }
    ...
    public static void getStringFieldsRecursively(final Class<?> clazz,
        ...
        getStringFieldsRecursively(clazz.getSuperclass(),
                stringFields,
                prefix);
        ...
        for (Field field : fields) {
            if (field.getType()
                ...
                if (collectionActualType.equals(String.class) || collectionActualType.isEnum()) {
                    stringFields.add(prefix.concat(field.getName()));
                } else if (collectionActualType.getSimpleName()
                        .toUpperCase(Locale.ROOT)
                        .endsWith("DTO") && !collectionActualType.equals(clazz)) {
                    getStringFieldsRecursively(collectionActualType,
                            stringFields,
                    ...
                    && !field.getType()
                            .equals(clazz)) {
                getStringFieldsRecursively(field.getType(),
                        stringFields,
                        Optional.ofNullable(prefix)
                                .orElse(StringUtils.EMPTY)
                                .concat(field.getName()
