...
public class PostBulkWorkflow extends AbstractBulkReindexWorkflow<Post, PostElasticDTO> {
    ...
    protected void initIndecies() {
        TAG_INDEX.put(properties.get("tag.vk.native"),
                properties.get("elastic.index.native"));
        TAG_INDEX.put(properties.get("tag.vk.sets"),
        ...
        TAG_INDEX.put(properties.get("tag.vk.art"),
    ...
    public void addEntityToBulkWithCustomIndex(final PostElasticDTO dto,
    ...
    public void addEntityToBulk(final PostElasticDTO dto,
                                BulkRequestBuilder bulkRequest,
                                BulkOptions bulkOption)
            throws JsonProcessingException {
        ...
        if (bulkOption == BulkOptions.UPDATE) {
            UpdateRequestBuilder indexBuilder = prepareUpdateIndex(dto,
                    elasticWorkflow.getDocumentId(dto.getId()));
            if (indexBuilder == null) {
                return;
            }
        ...
        if (bulkOption == BulkOptions.DELETE) {
            DeleteRequestBuilder indexBuilder = prepareDeleteIndex(dto,
                    elasticWorkflow.getDocumentId(dto.getId()));
            if (indexBuilder == null) {
                return;
            }
    ...
    protected IndexRequestBuilder prepareIndex(final PostElasticDTO dto,
                                               final String id,
                                               String index) {
        if (dto == null) {
            return null;
    ...
    protected UpdateRequestBuilder prepareUpdateIndex(final PostElasticDTO dto,
                                                      final String id) {
        if (dto == null) {
            return null;
        }
        ...
        try {
            String index = getIndex(dto.getText(),
                    TAG_INDEX);
            return index != null ? nativeElasticClient.getClient()
                    .prepareUpdate(index,
                            TYPE,
    ...
    protected DeleteRequestBuilder prepareDeleteIndex(final PostElasticDTO dto,
                                                      final String id) {
        if (dto == null) {
            return null;
        }
    ...
    public String getIndex(final String text,
                           Map<String, String> tag_index) {

        if (text == null) {
            return null;
        }

        Entry<String, String> filtered = tag_index.entrySet()
                .stream()
                .filter(map -> text.contains(map.getKey()))
                .findAny()
                .orElse(null);

        String index = Optional.ofNullable(filtered)
                .map(Entry::getValue)
                .orElse(null);
