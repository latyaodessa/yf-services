...
public class EmailService {
    ...
    public Response sendEmail(final EmailToSendDTO dto) {
        ClientResponse response = Client.create().resource(MAIL_SENDER_URI + "rest/send")
                .type(MediaType.APPLICATION_JSON)
                .post(ClientResponse.class, dto);

        if (response.getStatus() != 200) {
            LOG.severe("Failed : HTTP error code : " + response.getStatus() + " error " + response.getClient().getMessageBodyWorkers());
    ...
    public void sendVerificationEmail(final User userToRegister) {
        final Verifications verifications = verificationService.createEmailVerification(userToRegister, VerificationTypesEnum.EMAIL);
        final EmailToSendDTO emailToSendDTO = emailGenerator.generateVerifyEmail(verifications, userToRegister);
        sendEmail(emailToSendDTO);
    }

    public void sendResetPasswordEmail(final User user) {
        final Verifications verifications = verificationService.createEmailVerification(user, VerificationTypesEnum.RESET_PASSWORD);
        final EmailToSendDTO emailToSendDTO = emailGenerator.generateResetaPasswordEmail(verifications, user);
        sendEmail(emailToSendDTO);
    }
