...
public class EmailGenerator {
    ...
    public EmailToSendDTO generateVerifyEmail(final Verifications verifications,
                                              final User userToRegister) {
        final EmailTemplate emailTemplate = emailDAO.getTemplateByNameLanguage(MailTemplateNamesEnum.VERIFICATION,
                LanguagesEnum.RU);
        EmailToSendDTO dto = new EmailToSendDTO();
        dto.getPlaceHolders()
                .put("link",
                        DOMAIN.concat("verify/")
                                .concat(verifications.getVerification()));

        converter.templateToEmailSendDto(emailTemplate,
                dto,
                userToRegister);
        return dto;
    }

    public EmailToSendDTO generateResetaPasswordEmail(final Verifications verifications,
                                                      final User userToRegister) {
        final EmailTemplate emailTemplate = emailDAO.getTemplateByNameLanguage(MailTemplateNamesEnum.RESET_PASSWORD,
                LanguagesEnum.RU);
        EmailToSendDTO dto = new EmailToSendDTO();
        dto.getPlaceHolders()
                .put("link",
